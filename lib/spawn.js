...
function openWindowsTerminalApp(workDir, label, index) {
  let cmd
  if (IS_WSL) {
    // WSL: wsl.exe로 bash 열고 WSL 경로 그대로 사용 (경로 변환 불필요)
    const escapedPath = workDir.replace(/'/g, "'\\''")
    cmd = `wt.exe --window new new-tab --title "${label}" wsl.exe bash -c "cd '${escapedPath}' && claude; exec bash"`
  } else {
    cmd = `wt.exe --window new new-tab --title "${label}" --startingDirectory "${workDir}" cmd /k claude`
  }
  spawnDetached(cmd)
}
