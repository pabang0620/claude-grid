...
// PS1 파일을 /tmp 에 쓰고, WSL이면 UNC 경로로 변환해 powershell.exe 에 넘긴다
function runPs1Sync(script, timeout = 10000) {
  ...
  try {
    ...
    return execSync(
      `${POWERSHELL_CMD} -ExecutionPolicy Bypass -File "${ps1Path}"`,
...
function spawnPs1Async(script) {
  ...
  try {
    fs.writeFileSync(linuxTemp, script, 'utf-8')
    const ps1Path = IS_WSL ? toWindowsPath(linuxTemp) : linuxTemp
    const child = spawn(POWERSHELL_CMD, ['-ExecutionPolicy', 'Bypass', '-File', ps1Path], {
...
[WinSnapA]::EnumWindows([WinSnapA+EnumWindowsProc]{
    param($hwnd, $lParam)
...
function Get-NewWTHwnds {
    ...
    [WinPosB]::EnumWindows([WinPosB+EnumWindowsProc]{
        param($hwnd, $lParam)
