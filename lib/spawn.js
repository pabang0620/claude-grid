...
function Apply-Layout {
    for ($i = 0; $i -lt [Math]::Min($newHwnds.Count, $cells.Count); $i++) {
        $c = $cells[$i]
        [WinPosB]::SetWindowPos([IntPtr]$newHwnds[$i], [IntPtr]::Zero, $c.x, $c.y, $c.w, $c.h, 0x0014) | Out-Null
    }
}

# HWND가 모일 때까지 최대 8초 대기
$newHwnds = @()
for ($i = 0; $i -lt 27; $i++) {
...
# WT 초기화 완료 대기 — 여기서 기다려야 크기 변경이 정착됨
Start-Sleep -Seconds 8

# 3초 간격으로 3번 적용 (WT가 WM_SIZE로 되돌려도 재적용)
Apply-Layout
