...
# Get new WT processes not in prePids, sorted oldest→newest (matches window open order)
$newProcs = Get-Process -Name "WindowsTerminal" -ErrorAction SilentlyContinue |
            Where-Object { $_.Id -notin $prePids } |
            Sort-Object StartTime

# Retry until we have HWNDs for all new processes (up to 6 seconds)
$pidHwnd = @{}
$maxTries = 20
$try = 0
...
# Position each new window to its cell (oldest process = first window opened)
for ($i = 0; $i -lt [Math]::Min($newProcs.Count, $cells.Count); $i++) {
    $pid = $newProcs[$i].Id
    $cell = $cells[$i]
    $hwnd = $pidHwnd[[int]$pid]
    if ($hwnd) {
