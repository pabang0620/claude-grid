...
export async function spawnWindows(terminal, platform, windows) {
  for (let i = 0; i < windows.length; i++) {
    ...
    if (platform === 'windows' || platform === 'wsl') {
      await sleep(1500)
      positionWindowByPid(cell)
    } else {
      await sleep(OPEN_DELAY_MS)
    }
  }
  // Mac: osascript handles open+position in one call
  // Linux: no auto-positioning
}
...
function positionWindowByPid(cell) {
  try {
    runPowerShellScript(buildWindowsPosScriptByPid(cell))
  } catch {
    // 실패 무시 (--pos로 이미 위치 잡혀있음)
  }
}

function buildWindowsPosScriptByPid(cell) {
...
$maxTries = 20
$try = 0
while (-not $found -and $try -lt $maxTries) {
...
if ($found) {
    [Win32]::SetWindowPos($found, [IntPtr]::Zero, ${x}, ${y}, ${width}, ${height}, 0x0044)
}
`.trim()
}
