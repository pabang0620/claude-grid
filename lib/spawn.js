...
function openWindowsTerminalApp(workDir, label, index) {
  ...
  const cmd = `wt.exe --window new new-tab --title "${label}" --startingDirectory "${workDir}" cmd /k claude`
  spawnDetached(cmd)
}
...
function openCmd(workDir, label) {
  ...
  const cmd = `Start-Process cmd /ArgumentList "/k cd /d \\"${workDir}\\" && claude"`
  runPowerShell(cmd)
}
...
function runPowerShell(cmd) {
  try {
    execSync(`powershell -Command "${cmd.replace(/"/g, '\\"')}"`, { stdio: 'pipe' })
  } catch {
    // 비동기 실행 실패 무시
  }
}
...
function runPowerShellScript(script) {
  ...
  try {
    ...
    execSync(`powershell -ExecutionPolicy Bypass -File "${tempFile}"`, { stdio: 'pipe' })
  } catch {
    // 위치 조정 실패 무시
  } finally {
