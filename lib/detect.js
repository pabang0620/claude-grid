...
function detectWSLMonitors() {
  try {
    const script = `Add-Type -AssemblyName System.Windows.Forms; $screens = [System.Windows.Forms.Screen]::AllScreens; $result = @(); foreach ($s in $screens) { $result += "$($s.WorkingArea.X),$($s.WorkingArea.Y),$($s.WorkingArea.Width),$($s.WorkingArea.Height),$($s.Primary)" }; $result -join ';'`
...
foreach ($s in $screens) {
  $result += "$($s.WorkingArea.X),$($s.WorkingArea.Y),$($s.WorkingArea.Width),$($s.WorkingArea.Height),$($s.Primary)"
}
$result -join ';'
`
