...
function detectWSLMonitors() {
  try {
    ...
    const tmpFile = path.join(os.tmpdir(), `claunch-detect-${Date.now()}.ps1`)
    fs.writeFileSync(tmpFile, script, 'utf-8')
    const winPath = execSync(`wslpath -w "${tmpFile}"`, { stdio: 'pipe', encoding: 'utf-8' }).trim()
    const result = spawnSync('powershell.exe', ['-ExecutionPolicy', 'Bypass', '-NoProfile', '-File', winPath], {
